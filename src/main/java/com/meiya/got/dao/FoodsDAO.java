...
public interface FoodsDAO {
    String TABLE_NAME = "my_food";
    String INSET_FIELDS = "phone, password, salt";
    String SELECT_FIELDS = "id, store_id, name, photo, photo_sub, cate_id, description, stock, price, status, create_time";


    @Select({"select * from my_food where id = #{id}"})
    ...
    @Select({"select * from my_food where store_id = #{storeId}"})
    List<Foods> selectByStore(@Param("storeId")Long storeId);


    @Update({"update my_food set stock=#{stock} where id=#{id}"})
